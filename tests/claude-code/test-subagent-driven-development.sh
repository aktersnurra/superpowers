...
# Test 8: Verify jj workspace requirement
echo "Test 8: jj workspace requirement..."
...
if assert_contains "$output" "using-jj-workspaces\|jj workspace\|workspace" "Mentions jj workspace requirement"; then
	: # pass
else
	exit 1
fi
...
# Test 9: Verify root workspace warning
echo "Test 9: Root workspace red flag..."

output=$(run_claude "In subagent-driven-development, is it okay to start implementation directly in the root jj workspace?" 30)

if assert_contains "$output" "jj workspace\|workspace\|not.*root\|never.*root\|avoid.*root\|don't.*root\|consent\|permission" "Warns against root workspace"; then
	: # pass
else
	exit 1
fi
